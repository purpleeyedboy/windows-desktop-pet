[CmdletBinding()]
param([string]$CandidatePath = "dist-integrated/桌面宠物_全部功能合并测试版_20260915.exe")
$ErrorActionPreference = "Stop"
$candidate = (Resolve-Path -LiteralPath $CandidatePath).Path
$testRoot = Join-Path ([IO.Path]::GetTempPath()) ("integrated-startup-" + [guid]::NewGuid().ToString("N"))
...
using System.Text;
public static class IntegratedSmokeWindows {
...
try {
    ...
    $start.Environment["USERNAME"] = "IntegratedCI-" + [guid]::NewGuid().ToString("N")
    ...
    if (-not $ready.same_hunger -or -not $ready.single_drop_owner -or $ready.feed_frames -ne 6) { throw "Incomplete integrated ownership/feed frames" }
    if (-not $ready.groom.ready -or -not $ready.groom.same_runtime -or $ready.groom.sides.left.frame_count -ne 12 -or $ready.groom.sides.right.frame_count -ne 12) { throw "Groom not connected" }
    if ($ready.enabled_features.Count -ne 7) { throw "Not all features are enabled" }
    Get-Content -LiteralPath $marker -Raw | Write-Output
    ...
    $windows = @([IntegratedSmokeWindows]::Visible([int[]]@($ownedIds)))
    if (@($windows | Where-Object { $_ -match '无法启动|Fatal error|Traceback' }).Count) {
        throw "Candidate displayed an error window: $($windows -join ', ')"
    }
    if (-not @($windows | Where-Object { $_ -like 'TkTopLevel|桌面宠物 V2.1-ALL |*' }).Count) {
        throw "No identified visible Tk candidate window in the launched process tree: $($windows -join ', ')"
    }
    "Isolated candidate process tree displayed its identified Tk window after twelve seconds; pending user visual acceptance." |
