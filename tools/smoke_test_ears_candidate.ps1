...
using System.Text;
public static class EarsSmokeWindows {
...
try {
    ...
    $start.Environment["USERNAME"] = "EarsCI-" + [guid]::NewGuid().ToString("N")
    ...
    $windows = @([EarsSmokeWindows]::Visible([int[]]@($ownedIds)))
    if (@($windows | Where-Object { $_ -match '无法启动|Fatal error|Traceback' }).Count) {
        throw "Candidate displayed an error window: $($windows -join ', ')"
    }
    if (-not @($windows | Where-Object { $_ -match '^TkTopLevel\|桌面宠物 \| .+ \| .+ \| 基础 .+ \| V2\.1-EARS 测试$' }).Count) {
