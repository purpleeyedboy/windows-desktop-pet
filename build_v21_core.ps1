...
Push-Location $root
try {
    & $python tools/verify_about_localization.py
    if ($LASTEXITCODE -ne 0) { throw "About localization verification failed" }
    ...
    $identity = @{ product_version = "2.1.0"; build_date = $buildDate; git_short_hash = $fullHash; foundation_commit = $fullHash; enabled_features = @("common-foundation-runtime"); test_build = $true; debug_enabled = $true; debug_menu_enabled = $true } | ConvertTo-Json -Compress
    [IO.File]::WriteAllText((Join-Path $work "build_identity.json"), $identity, [Text.UTF8Encoding]::new($false))
    $description = "V2.1-CORE; common-foundation; Test build: true; Debug menu: true; BASE-001; Build $buildDate; Git $shortHash"
    $versionText = @"
