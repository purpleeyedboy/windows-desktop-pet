...
& $python tools\v21_paws_gate.py
if ($LASTEXITCODE -ne 0) { throw 'V2.1-PAWS 文本差异与基线素材哈希验证失败' }
...
$expectedExe = Join-Path $projectRoot 'dist\桌面宠物_双前肢按压鼠标.exe'
...
& $python tools\verify_release_archive.py $expectedExe --require-paws
if ($LASTEXITCODE -ne 0) { throw 'PyInstaller 单文件归档资源验证失败' }

$exeFiles[0] | Select-Object FullName, Length, LastWriteTime

$hash = Get-FileHash -Algorithm SHA256 -LiteralPath $expectedExe
