...
- Four 100-entry short Chinese mood corpora (10 complete thoughts × 10 affectionate endings), mild/severe/critical shake levels.
- Pending hunger animation waits for the coordinator and starts its text at the same time.

Evidence: 5 focused tests pass, compileall passes, shared-state verification passes, 15 inherited foundation blobs verified. Existing test_hunger.py cannot collect because its old HungerStore API no longer exists in the inherited branch. No display/Xvfb or Windows runtime is available here, so HUD placement and bubble motion require Windows verification.
