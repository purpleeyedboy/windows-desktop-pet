...
from .hunger_window import HungerWindow as PetWindow
...
def main() -> int:
    ...
    try:
        ...
        hunger_service = HungerService(SharedHungerState(services), utc_clock.utc_seconds)
        hunger_runtime = HungerGraphicRuntime(
            services=services,
            service=hunger_service,
            window=pet_window,
            clock=services.runtime.clock,
        )
        artwork = load_hunger_graphic_assets(pet_window._neutral_center_frame)
        pet_window.attach_hunger(hunger_runtime, artwork, utc_clock)
        root.title(f"桌面宠物 V2.1-HUNGER | {build_info.git_short_hash}")
