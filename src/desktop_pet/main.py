...
from .assets import load_frames, load_feed_frames, load_head_neck_compositor, load_playback_sequences
...
from .feed_core.runtime import FeedRuntime, FeedDragDropService
...
def load_feed_build_info() -> BuildInfo:
    payload = json.loads(asset_path('BUILD_INFO_FEED_CORE.json').read_text(encoding='utf-8-sig'))
    return BuildInfo(payload['version'], date.fromisoformat(payload['date']),
        payload['git_short_hash'], payload['foundation_actual_commit'],
        FeatureConfig(enabled_features=('common-foundation', 'feed'),
            test_build=bool(payload['test_build']), debug_enabled=False,
            debug_menu_enabled=bool(payload['debug_menu'])))
...
def main() -> int:
    ...
    build_info = load_feed_build_info()
    ...
    try:
        ...
        services.dragdrop = FeedDragDropService(feed_runtime)
        frames = load_frames()
        ...
        feed_runtime.attach_window(pet_window, load_feed_frames())
        if os.environ.get('DESKTOP_PET_SMOKE_CHECK') == '1' and build_info.feature_config.test_build:
            def report_smoke_ready():
                marker = services.paths.root / 'smoke-ready.json'
                with marker.open('x', encoding='utf-8') as stream:
                    json.dump({'ready': True, 'feed_frames': len(pet_window.frames['feed']),
                        'same_runtime': pet_window.services is feed_runtime.services,
