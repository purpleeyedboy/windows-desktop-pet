...
if($LASTEXITCODE -ne 0){throw 'PR5 foundation and FEED handler are required; refusing to package an incomplete candidate'}
...
$exes=@(Get-ChildItem dist-feed-core -Filter *.exe -File);if($exes.Count -ne 1){throw "Expected exactly one EXE; found $($exes.Count)"};if($exes[0].Name -ne '桌面宠物_文件喂食与回收站事务修复.exe'){throw 'Unexpected EXE'}
