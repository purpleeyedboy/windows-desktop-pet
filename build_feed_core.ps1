...
foreach($foundationFile in @('src\desktop_pet\foundation\services.py','src\desktop_pet\foundation\runtime.py')){if(-not(Test-Path $foundationFile)){throw "Missing codex-od26j1 e178f371 foundation file: $foundationFile"}}
& $python tools\check_feed_foundation_gate.py
if($LASTEXITCODE -ne 0){throw 'Published foundation services and trusted FEED handler are required; refusing to package an incomplete candidate'}
& $python -m compileall -q src run_desktop_pet.py tools\build_feed_frames.py tools\verify_feed_core_archive.py
if($LASTEXITCODE -ne 0){throw 'Python compile check failed'}
foreach($required in @('assets\keyframes','assets\generated\work\feed\v1','assets\feed\v1\manifest.json','assets\feed\v1\source\feed-sources.base64.txt','assets\bubble','assets\fonts','assets\dialogue','assets\rig\v1\source\eye-neutral-v1','THIRD_PARTY_NOTICES.txt')){if(-not(Test-Path $required)){throw "Missing resource: $required"}}
