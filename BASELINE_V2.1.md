...
## REPAIR-20260907-V21-HUNGER 状态

- 撤回此前饥饿功能完成结论：旧 0..10000/冻结调试时钟/几何弧线实现已返工。
- 状态实现使用 0..100000、真实 UTC 120 分钟衰减、四态边界与统一状态 adapter。
- 公共基础源码已通过真实 `ApplicationServices` API 接入；旧几何表情替代已移除，但真实逐帧嘴舌/泪滴素材尚缺，因此当前不发布候选。素材接入后最终 Windows 验收仍由用户本人执行。
- 2026-09-08 已加入严格素材导入器与示例帧表：仅接收 RGBA、校验确认默认帧 SHA-256、禁止隐式缩放，并把 512×768 美术画布以明确 `[64,0]` 变换放入 640×768 运行画布；它可生成无损运行资源、联系表、逐段 GIF 与帧清单。当前仍无正式生成帧，工具演练不作为美术或动画验收。
