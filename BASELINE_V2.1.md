...
- 可见合成修正：`side/phase/arm/tongue` 已接入既有头眼帧之后、透明窗口之前的确定性 RGBA 合成；中性帧保持对象和字节不变，失焦、输入、动作、异常及退出均释放舔手姿态。QA PNG 仅生成到调用者指定的临时目录，不进入仓库。
- 保留范围：未修改任何素材、黄金图、转头角度、Alpha、头颈、眼球跟随或 minimum-jerk；未实现耳朵、双前肢、饥饿、拖放或喂食。
- 独立候选：`桌面宠物_空闲随机舔手.exe`；构建时嵌入完整构建信息，Windows Actions 不运行 pytest，只检查 PyInstaller、唯一 EXE、50 MiB 上限和 SHA-256 后上传该候选。
- 自动化状态：可见舔手/状态机/运行时/发布合同聚焦测试 121 项通过，窗口 headless 测试 33 项通过，头眼/头颈/眨眼/歪头运动回归 163 项通过，Python 编译和 `git diff --check` 通过。相对 `c3b218d` 的 19 个差异文件均为文本，既有素材、QA 金图和交付目录无变化。历史完整套件的依赖、DISPLAY 和旧黄金视觉限制仍只记录，不删除测试或修改金图。
- 待完成门：Windows Actions 构建、候选下载后 SHA-256 复核、真实 Windows 桌面运行及用户视觉验收；Linux 容器不替代这些门。
- 候选门调整（2026-09-05）：按用户决定，旧基线自动测试不再阻塞 `桌面宠物_空闲随机舔手.exe`。Windows workflow 删除 pytest，直接以 `-SkipTests` 构建，只门禁 PyInstaller 成功、唯一 EXE、大小和 SHA-256 并上传 artifact；候选标记为未自动测试、等待用户 Windows 实机验收。
