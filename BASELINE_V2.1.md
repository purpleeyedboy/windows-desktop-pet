...
- 公共基础恢复修复的已发布来源提交记录为 `77a4f3a90c74c549a66052bebb4b652500e1f7be`；候选构建脚本、运行时身份回退和 Windows workflow 使用同一完整 SHA，不再标记旧的 `4eda8964...` 候选来源。
- 当前隔离环境无法连接 GitHub，因此不能在本地复核该提交的 Actions 页面或重新下载远端提交内容；本轮没有据此伪称完成远端差异审计。既有双耳接线及已验收素材保持不变。
- Windows workflow 在打包前运行左右耳按住/释放、指针离开、失焦、菜单中断、迟到回调和透明区穿透回归；通过后仍只构建并上传一个耳朵单功能 EXE。自动化、Windows 构建/artifact、实机交互和视觉验收继续分别记录，后两项仍待用户完成。
