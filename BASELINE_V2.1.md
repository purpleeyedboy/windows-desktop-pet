...
- Windows Actions 负责完整测试、唯一 EXE 校验、大小与 SHA-256 输出和单文件 artifact 上传。Windows 构建、真实桌面运行与用户视觉验收在 PR 后仍是独立待完成门禁。
...
- PR 二进制门禁：相对 V2.1 基线的全部变更必须是文本；耳朵预览仅生成到测试或 CI 临时目录并作为 artifact 上传。全部 158 个既有 `assets/` 文件以 Git blob 哈希逐项锁定，双耳逐像素命中、DPI、透明边缘与独立 EXE 契约继续由发布测试覆盖。
