...
def test_drag_candidate_metadata_is_complete_and_scope_limited():
    ...
    assert metadata == {
        "version": "2.1-drag-repair-dev.1",
        "date": "2026-09-06",
        "git_short_hash": "BUILD_TIME_REQUIRED",
        "foundation_commit": "NOT_INTEGRATED",
        "baseline": "BASE-001 / V2.1",
        "enabled_features": ["approved-v2.1-baseline"],
        "incomplete_features": ["file-drag-expectation-feedback-foundation-wiring"],
        "test_build": True,
        "debug_menu": True,
        ...
        "candidate_status": "blocked-awaiting-pr5-foundation-integration",
    }
...
def test_drag_candidate_spec_is_onefile_with_exact_name_and_metadata():
    ...
    assert "name='桌面宠物_文件拖动期待反馈修复'" in spec
...
def test_drag_build_and_actions_skip_tests_but_enforce_candidate_artifact_gates():
    ...
    assert "foundation integration required" in version_info
    assert "Automated tests: NOT RUN; Windows acceptance NOT RUN" in workflow
    assert "Foundation integration: NOT COMPLETE" in workflow
...
def test_debug_menu_and_docs_name_the_feedback_only_candidate():
    ...
    assert "开始拖动期待演示" in window
    assert "结束拖动期待演示" in window
